...
Current Contact, Address, and Employment Info.</h5>
 ...
 <div class="col">
   <table class="table-list">
		...
		<tr>
			<td></td>
			<td style="color: red; text-align: centre;">Blacklisted by <?php echo $blackListed->authorizeby." for ".$blackListed->reason;?></td>
		</tr>
		<?php } ?>
    </table>              
 </div><br />  
...
<?php 
 if(!is_object($report->ActiveDirectorAddressHistory)){
	...
	if($report->CommercialActivePrincipalInformation->DirectorID == $ActiveDirectorAddressHistory->DirectorID){
		?>
		<tr class="border_bottom">
	 ...
	 if($report->CommercialActivePrincipalInformation->DirectorID == $report->ActiveDirectorAddressHistory->DirectorID){
	 ?>
	<tr class="border_bottom">
...
<?php if(is_object($report->DirectorJudgments) && ($report->DirectorJudgments->DirectorID == $report->CommercialActivePrincipalInformation->DirectorID)){
		  $name = "";
...
<?php } else { 
	foreach($report->DirectorJudgments as $DirectorJudgments){
		if($report->CommercialActivePrincipalInformation->DirectorID == $DirectorJudgments->DirectorID){
		  $name = "";
 ...
 <hr class="hr-line"/>	
	<?php } }
		...
		<div class="panel-heading"><strong>Judgment - (No Data Available)</strong></div>
		<hr class="hr-line"/>	
<?php } ?>
 <!-- DirectorJudgments -->
